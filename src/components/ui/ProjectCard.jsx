...
import ImgModal from "../ImageModal.jsx"
...
export default function ProjectCard({ title, category, location, image, index = 0 }) {
  ...
  return (
    ...
    >
      <div className="overflow-hidden" onClick={() => setImgModal(true)}>
        <img
          src={image}
          alt={title}
          loading="lazy"
          className="w-full h-64 md:h-72 object-cover scale-100 cursor-pointer group-hover:scale-110 transition-transform duration-700 ease-out"
        />
      </div>
      <div className="absolute inset-0 bg-gradient-to-t from-background via-background/20 to-transparent opacity-70 group-hover:opacity-90 transition-opacity duration-500 pointer-events-none" />
